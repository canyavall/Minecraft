...
function getComplexityEffort(complexity) {
    const efforts = {
        "Simple": "2-3 days",
        "Medium": "3-5 days",
        "Complex": "5-7 days",
    ...
    return efforts[complexity] || "TBD";
}

function generateRequirements(num, name, category, biomes, complexity, features) {
    const epicNum = String(num + 10).padStart(2, '0');
    const effort = getComplexityEffort(complexity);
    const slugName = slug(name);
    const className = name.replace(/ /g, "").replace(/'/g, "");

    return `# Epic ${epicNum}: ${name} - ${category} Animal
...
1. **Content Addition** - Adds unique wildlife to Minecraft world
   - Players encounter ${name.toLowerCase()} in ${biomes.toLowerCase()}
   - Enriches biodiversity and exploration rewards
   - Creates memorable encounters

2. **Authentic Behavior** - Behaves according to its category (${category.toLowerCase()})
...
1. **3D Animated Model**
   - Accurate anatomy for ${name.toLowerCase()}
...
- [ ] ${name} entity is summonable (\`/summon xeenaa-alexs-mobs:${slugName}\`)
...
- ${className}Model class (GeckoLib model)
- ${className}Renderer class (GeckoLib renderer)
- Geometry file (${slugName}.geo.json)
- Animation file (${slugName}.animation.json)
- Texture file(s) (${slugName}.png + variants)
...
### Resource Files
- \`${slugName}.geo.json\`
- \`${slugName}.animation.json\`
- \`${slugName}.png\` (+ variants)
- Sound files
...
function main() {
    const basePath = path.join('C:', 'Users', 'canya', 'Documents', 'projects', 'Minecraft', 'xeenaa-alexs-mobs', '.claude', 'epics');

    // Parse mob data
    const mobs = [];
    const lines = MOBS_DATA.trim().split('\n');
    ...
    console.log(`Processing ${mobs.length} mobs...`);

    for (const mob of mobs) {
        // Epic number starts at 11 (we have 01-10 already)
        const epicNum = String(mob.num + 10).padStart(2, '0');
        const slugName = slug(mob.name);
        const categorySlug = getCategorySlug(mob.category);

        const folderName = `${epicNum}-${slugName}-${categorySlug}`;
        ...
        console.log(`✅ Created ${folderName}/requirements.md`);
    }

    console.log(`\n✅ Successfully created ${mobs.length} epic folders!`);
}
