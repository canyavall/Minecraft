...
def get_category_description(category):
    """Get description for behavior category"""
    descriptions = {
        "Passive": "passive",
        "Neutral": "neutral",
        "Defensive": "defensive",
...
2. **Authentic Behavior** - Behaves according to its category ({category_desc})
...
- {name.replace(" ", "")}Entity class
- {name.replace(" ", "")}Model class (GeckoLib model)
- {name.replace(" ", "")}Renderer class (GeckoLib renderer)
...
### Code Files
- `{name.replace(" ", "")}Entity.java`
- `{name.replace(" ", "")}Model.java`
- `{name.replace(" ", "")}Renderer.java`
...
- Catalog entry: CATALOG.md line {num + 33}
...
def main():
    base_path = "xeenaa-alexs-mobs/.claude/epics"
    ...
    for mob in mobs:
        ...
        category_slug = slug(get_category_description(mob['category']))
