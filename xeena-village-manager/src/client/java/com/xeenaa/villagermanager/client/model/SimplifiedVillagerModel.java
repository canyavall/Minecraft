...
public class SimplifiedVillagerModel extends EntityModel<VillagerEntity> implements ModelWithArms {
    ...
    public void setAngles(VillagerEntity entity, float limbAngle, float limbDistance, float animationProgress, float headYaw, float headPitch) {
        ...
        // Check if entity is using a bow (has active item)
        boolean isUsingBow = !entity.getActiveItem().isEmpty() &&
                             entity.getActiveItem().getItem() instanceof net.minecraft.item.BowItem;

        if (isUsingBow) {
            // BOW DRAW ANIMATION
            // Calculate how long the bow has been drawn (0.0 to 1.0)
            int itemUseTime = entity.getItemUseTime();
            float pullProgress = Math.min(1.0f, itemUseTime / 20.0f); // 20 ticks = fully drawn

            // Determine which hand is holding the bow
            net.minecraft.util.Arm arm = entity.preferredHand == net.minecraft.util.Hand.MAIN_HAND
                ? entity.getMainArm()
                : entity.getMainArm().getOpposite();

            boolean rightHanded = arm == net.minecraft.util.Arm.RIGHT;

            if (rightHanded) {
                // Right arm pulls the bow string
                this.rightArm.pitch = -0.9F - pullProgress * 0.3F; // Pull back as bow is drawn
                this.rightArm.yaw = -0.3F;
                this.rightArm.roll = 0.0F;

                // Left arm holds the bow out
                this.leftArm.pitch = -0.97F; // Point forward
                this.leftArm.yaw = 0.3F;
                this.leftArm.roll = 0.0F;
            } else {
                // Left arm pulls the bow string
                this.leftArm.pitch = -0.9F - pullProgress * 0.3F; // Pull back as bow is drawn
                this.leftArm.yaw = 0.3F;
                this.leftArm.roll = 0.0F;

                // Right arm holds the bow out
                this.rightArm.pitch = -0.97F; // Point forward
                this.rightArm.yaw = -0.3F;
                this.rightArm.roll = 0.0F;
            }
        } else {
            ...
            if (entity.handSwingProgress > 0.0F) {
                ...
                net.minecraft.util.Arm swingArm = entity.preferredHand == net.minecraft.util.Hand.MAIN_HAND
                    ? entity.getMainArm()
                    : entity.getMainArm().getOpposite();

                ModelPart swingingArm = swingArm == net.minecraft.util.Arm.RIGHT ? this.rightArm : this.leftArm;
