...
public class ProfessionButton extends ButtonWidget {
    ...
    public void renderWidget(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        int overlayColor = 0;
        if (!this.active) {
            overlayColor = 0x40000000; // Darker overlay when disabled
        } else if (this.isSelected()) {
            overlayColor = 0x20FFFF00; // Slight yellow overlay when selected
        } else if (this.isHovered()) {
            overlayColor = 0x20FFFFFF; // Slight white overlay when hovered
        }

        if (overlayColor != 0) {
            context.fill(this.getX() + 1, this.getY() + 1,
