...
public class GuardMeleeAttackGoal extends MeleeAttackGoal {
    ...
    private static final int SWING_ANIMATION_DELAY = 3; // Ticks before damage (0.15 seconds)
    ...
    public void tick() {
        ...
        // Look at target - increased turn speed for more responsive combat feel
        guard.getLookControl().lookAt(target, 40.0f, 40.0f);
    ...
    protected void attack(LivingEntity target) {
        ...
        // If animation not started, trigger swing animation
        if (animationTimer <= 0) {
            // Trigger arm swing animation BEFORE damage
            guard.swingHand(Hand.MAIN_HAND);

            // Set animation timer to sync damage with swing
            animationTimer = SWING_ANIMATION_DELAY;

            // Log animation trigger
            LOGGER.info("[MELEE ANIMATION] Guard {} started swing animation at {:.2f} blocks",
                guard.getUuid().toString().substring(0, 8),
                actualDistance);
        ...
        // Animation in progress but not complete yet
        if (animationTimer > 1) {
            return; // Keep waiting
        }

        // Animation complete (animationTimer == 1), deal damage
        ...
        // Set attack cooldown based on tier and reset animation timer
    ...
    private void performBasicMeleeAttackGuard(LivingEntity target, int tier) {
        ...
        // Animation is already triggered in attack() method before damage is dealt
